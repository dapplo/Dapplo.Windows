...
namespace Dapplo.Windows.Desktop
{
    ...
    public class WindowScroller
    {
        ...
        private bool ApplyPosition(ref ScrollInfo scrollInfo)
        {
            if (ShowChanges)
            {
                User32.User32Api.SetScrollInfo(ScrollBarWindow.Handle, ScrollBarType, ref scrollInfo, true);
            }
            switch (ScrollBarType)
            {
                case ScrollBarTypes.Horizontal:
                    User32.User32Api.SendMessage(ScrollingWindow.Handle, WindowsMessages.WM_HSCROLL, 4 + 0x10000 * scrollInfo.Position, 0);
                    break;
                case ScrollBarTypes.Vertical:
                case ScrollBarTypes.Control:
                    User32.User32Api.SendMessage(ScrollingWindow.Handle, WindowsMessages.WM_VSCROLL, (int) ((uint) ScrollBarCommands.SB_THUMBPOSITION + (scrollInfo.Position << 16)), 0);
                    break;
            }
            return true;
        }
        ...
        public bool GetPosition(out ScrollInfo scrollInfo)
        {
            ...
            return User32.User32Api.GetScrollInfo(ScrollBarWindow.Handle, ScrollBarType, ref scrollInfo);
        }
        ...
        public ScrollBarInfo? GetScrollbarInfo(bool forceUpdate = false)
        {
            ...
            var hasScrollbarInfo = User32.User32Api.GetScrollBarInfo(ScrollBarWindow.Handle, objectId, ref scrollbarInfo);
            if (!hasScrollbarInfo)
            {
        ...
        private bool SendScrollMessage(ScrollBarCommands scrollBarCommand)
        {
            ...
            {
                case ScrollBarTypes.Horizontal:
                    User32.User32Api.SendMessage(ScrollingWindow.Handle, WindowsMessages.WM_HSCROLL, scrollBarCommand, 0);
                    return true;
                case ScrollBarTypes.Vertical:
                    User32.User32Api.SendMessage(ScrollingWindow.Handle, WindowsMessages.WM_VSCROLL, scrollBarCommand, 0);
                    return true;
                default:
                    return false;
            }
